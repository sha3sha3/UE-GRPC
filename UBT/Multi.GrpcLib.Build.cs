...
using System.IO;
	...
	public class GrpcLib : ModuleRules
	{
		...
		else if (Target.Platform == UnrealTargetPlatform.Linux)
        {
			...
			/*
			 Copy required dynamically linked libs to the build folders (plugin and game)
			 */
            RuntimeDependencies.Add("$(PluginDir)/Binaries/Linux/libabsl_random_internal_seed_material.so.2206.0.0", Path.Combine(DynamicLibLinux64, "libabsl_random_internal_seed_material.so"));
            RuntimeDependencies.Add("$(TargetOutputDir)/libabsl_random_internal_seed_material.so.2206.0.0", Path.Combine(DynamicLibLinux64, "libabsl_random_internal_seed_material.so"));
